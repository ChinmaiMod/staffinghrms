 ...
 */
function MyTickets({ testMode = false, emptyMode = false, errorMode = false }) {
  ...
  const fetchTickets = async () => {
    try {
      ...
      //   .select(`
      //     *,
      //     employee:hrms_employees(first_name, last_name, employee_code)
      //   `)
      //   .eq('tenant_id', tenant?.id)
      //   .eq('business_id', selectedBusiness?.id || null)
      //   .eq('employee_id', user?.id) // Only tickets for current employee
    ...
    return tickets.filter(ticket => {
      ...
      if (selectedStatus !== 'all') {
        if (selectedStatus === 'open') {
          const openStatuses = ['ticket_created', 'in_team_review', 'need_leadership_discussion', 
                               'need_attorney_discussion', 'need_team_discussion', 'sent_to_candidate_review']
  ...
  const handleViewTicket = (ticketId) => {
    navigate(`/hrms/my-tickets/${ticketId}`)
  }
    ...
    <div className="my-tickets-container">
      ...
      {error && (
        <div className="error-message">
          ...
          <button onClick={fetchTickets} className="btn-primary">Retry</button>
        </div>
      )}
            ...
            <article key={ticket.ticket_id} className="ticket-card" role="article">
              ...
              {ticket.unread_comments > 0 && (
                <div className="ticket-notification">
                  💬 {ticket.unread_comments} new comment{ticket.unread_comments > 1 ? 's' : ''} since your last view
                </div>
              )}
